...
# ── Flex Message 漲幅排行卡片（波士頓奶油黃 × 亮曦黃金） ────────────────────────

def build_flex_leaderboard(growth_list, now_tw, market_close=None, market_chg=None, market_chg_pct=None, market_news=None):
    COLOR_HEADER = "#c9a84c"   # 亮曦黃金
    COLOR_TITLE  = "#fff8e8"   # 標題文字
    COLOR_SUB    = "#fff0b0"   # 副標題（加亮）
    COLOR_DATE   = "#fff8e8"   # 日期
    COLOR_PCT    = "#8b5e00"   # 百分比
    COLOR_RANK   = "#bbaa88"   # 圈號
    COLOR_NAME   = "#333333"   # 股票名稱
    COLOR_REC    = "#c9a84c"   # 推薦符號
    COLOR_FOOT   = "#b8952a"   # footer
    COLOR_NOTE   = "#b8952a"   # 符號說明
    COLOR_DIV    = "#e8d5a0"   # 分隔線
    ...
    for i, item in enumerate(growth_list):
        ...
        rec = str(item.get("rec", "")).strip()

        # 名稱欄：推薦符號佔位 + 股票代號 + 名稱（統一對齊）
        if rec == "★":
    ...
    flex_message = {
            ...
            "contents": [
                {
                    ...
                    "contents": [
                        ...
                        {"type": "text", "text": date_str, "size": "xs", "color": COLOR_DATE, "align": "end", "flex": 1, "gravity": "center"},
                    ]
                },
        ...
        "body": {
            ...
            "contents": [
                ...
                *([
                    {
                        ...
                        "contents": [
                            ...
                            *([{"type": "text", "text": "・" + n, "size": "xxs", "color": "#7a6030", "margin": "xs", "wrap": True} for n in (market_news or [])]),
                        ]
                    },
                ...
                *rows,
                {"type": "separator", "margin": "sm", "color": COLOR_DIV},
                {"type": "text", "text": "符號說明：★ 主要推薦購買　☆ 次要推薦購買", "size": "xxs", "color": COLOR_NOTE, "margin": "sm", "wrap": True},
                {"type": "text", "text": "Family · " + time_str, "size": "xxs", "color": COLOR_FOOT, "align": "end", "margin": "xs"},
            ]
        }
    }
    return flex_message
...
def job():
    ...
    for _, row in df_watch.iterrows():
        ...
        growth = None
        rec = str(row.get("推薦", "")).strip()
        if p is not None and p > 0 and cost_226 > 0:
            growth = (p - cost_226) / cost_226 * 100
            growth_list.append({"sid": sid, "name": name, "growth": growth, "rec": rec})
    ...
    for d in watch_data:
        ...
        if ex_date and cash:
            report += f"  💰除息：{ex_date} 現金股利：{cash}元\n"
